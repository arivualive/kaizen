...
$( '#genre' ).change( function (e) {
  ...
  .done ( function ( data ) {
	...
	if(download_file == 'json/dummy') {
		...
		$('#multi-csv').attr({href:download_file + 'd.csv', download:junleSelect + '-詳細' + today + '.csv'});
	}
	//$('#single-csv').text('CSV出力'); //ダウンロードリンク表示
	//$('#multi-csv').text('詳細CSV出力'); //ダウンロードリンク表示


    $( "#update-time" ).text('データ更新 - ' + return_data.update_time); //更新時間を挿入
    ...
    $( "#example-table" ).t({
      ...
      rowClick:function(e, row){
        ...
        $( "#category" ).html( "【コンテンツカテゴリー】"+junleSelect );
      ...
      if ( categoryNumber != "0" ) {
        ...
        downloadCsv ( studentResultsData );
          ...
          if ( secoundNumber < firstNumber ) {
            $( "#errmsg" ).html( "※左より大きい数字を入力してください" );

          } else if ( firstNumber == "" ) {
            $( "#errmsg" ).html( "※左側の数字を入力してください" );

          } else if ( secoundNumber == "" ) {
            $( "#errmsg" ).html( "※右側の数字を入力してください" );
