...
*/
  ...
  for ( $log = 0; $log < count( $log_data ); $log++ ) {
    ...
    if ( !empty( $returned_json[ 'datas' ][ 0 ][ 'logs' ] ) ) {
      ...
      //$hoge2 = call_user_func_array(array($curl, 'send'), $data);

      debug_r( 'hoge2' );
      debug_r( $log_data[ $log ][ 'history_id' ] );
      debug_r( $hoge2 );
  ...
  debug_r( 'contents_ids' );
  debug_r( $contents_ids );

  $cid_unique = array_unique( $contents_ids );

  $cid_unique = array_filter( $cid_unique, 'strlen' );
  /*debug_r( 'cid_unique' );
  debug_r( $cid_unique );
  */
  ...
  /*
  debug_r( 'student_id' );
  debug_r( $student_data[ 'student_id' ] );
  debug_r( $align_unique[ 'school_contents_id' ] );
  */
  $data[ 'method' ] = 'reaquireProportionData';
  $data[ 'params' ] = $align_unique;
  $history_data = $curl->send( $data );
  /*
  debug_r( 'history_data' );
  debug_r( $history_data );
  */
