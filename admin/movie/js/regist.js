$(function() {
    ...
    $('input[name=contents_file]').change(function() {
        ...
        if(file.size < (150 * 1024 * 1024)) {
            $('#contents_file_info').html('file name: ' + file.name + ' / size: ' + getFiseSize(file.size) + ' / type: ' + file.name.split('.')[file.name.split('.').length - 1]);
            $('#contents_file_info').css('color', '');
            $('#contents_not_file').remove();
            console.log(file.name.split('.')[file.name.split('.').length - 1]);
        } else {
            $('#contents_file_info').html('File size is over 150 MB: (Target file size)' + getFiseSize(file.size));
    ...
    $('input[name=attach_file]').change(function() {
        var file = $(this).prop('files')[0];
        if(file.size < (150 * 1024 * 1024)) {
            $('#attach_file_info').html('file name:' + file.name + ' / size:' + getFiseSize(file.size) + ' / type:' + file.type);
