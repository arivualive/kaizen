...
//---CSV output(レポート分布) ここから---
$csv_strings1 = array();

//CSV整形-レポート項目[A1-F1]
$csv_strings1[0] .= "Question number-Question sentence,Answer form,Required answer/Optional answer,Number of respondents,item(Single/Multiple)・Value(Number),Number of responses(Single/Multiple/Number)";
...
for($i = 0 ; $i < count($queryData) ; $i++) {
    if($queryData[$i]['query_type'] == 0 || $queryData[$i]['query_type'] == 1) {
        for($j = 0 ; $j < count($queryData[$i]) - 6 ; $j++) {
            ...
            if($j == 0) {
                $csv_strings1[$line_count] .= "Q" . ($i + 1) . "." . check_string_replace($queryData[$i]['query']) . ",";       //Question number-Question sentence
            ...
            $csv_strings1[$line_count] .= $queryData[$i][$j]['answer_count'] . ",";                                             //Number of responses(単一/複数/数値)
        }
        ...
        for($j = 0 ; $j < count($queryData[$i][0]) - 7 ; $j++) {
          ...
          if(is_countable($csv_strings1)){
            ...
            $csv_strings1[$line_count] .= $queryData[$i][0][$j]['count'] . ",";                                                 //Number of responses(単一/複数/数値)
        }
    } else {
        // 2019/6/03 count関数対策
        $line_count = 0;
        ...
        $csv_strings1[$line_count] .= "-";                                                                                      //Number of responses(単一/複数/数値)
    }
}
...
$csv_strings2[2] .= "Required answer/Optional answer,-,-,";
$csv_strings2[3] .= "Choice(Single/Multiple)・ Minimum label[value]～Maximum value label[value](Number),-,-,";

//CSV整形-レポートデータ[D1-*4]
for($i = 0 ; $i < count($queryData) ; $i++) {
    $csv_strings2[0] .= check_string_replace("Q" . ($i + 1) . "." . $queryData[$i]['query']) . ",";                             //Question number-Question sentence
    ...
    <div id="header">
        ...
        <div id="topicpath">
            <ol>
                ...
                <li class="active"><a>Report analysis</a></li>
            </ol>
        </div>
        <!-- ▼user information -->
    ...
    <div id="maincontents">
        ...
        <div id="box-questionnaire-importantinfo">
            ...
            <dl class="number-question">
                ...
                <dd><span><?php if(is_countable($queryData)){ echo count($queryData); } /*echo count($queryData);*/ ?></span> Q</dd>
            </dl>
