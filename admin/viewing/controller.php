    ...
    public function getContentLog($url, $histories, $send_data )
    {
        ...
        foreach ($histories as $key => $history)
        {
          ...
          if(is_countable($histories[$key])){
            $graph = [
                'balloonText' => $history[ 0 ][ 'history_id' ]."</br>Regeneration date : ".(count($histories[$key]) > 0 ? $histories[$key][0]['play_start_datetime'] : '')."<br>Content time  [[value]]</br>" . "Play time [[category]]",
                "bullet" => 'round',
                'bulletSize' => 10,
                'lineThickness' => 3,
