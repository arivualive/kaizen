...
foreach($csvRowD as  $key => $value) {
	$search .= '<option value="' . $key . '"';
	foreach((array)$_POST['search'] as $line) { if($line == $key) { $search .= ' selected'; } }
	$search .= '>' . $csvRowD[$key] . '（' . $key . '）</option>' . "\n";
}

$icon = ($_POST['search']) ? ((search_value($_POST['search'], $_POST['verify'], $select)) ? 'fas fa-lightbulb' : 'fas fa-bomb') : 'fas fa-balance-scale';
?>
<script>
...
<input type="text" size="44" name="string" pattern="^[0-9a-f-]+$" value="<?php echo ($_POST['flag']) ? $string : ((isset($_POST['string'])) ? $_POST['string'] : $database) ?>" placeholder="00000000-00000000-00000000-00000000-00000000" spellcheck="false" required><br>
<br>
<label for="and" class="item"><input type="radio" name="verify" value="0" class="margin line2" id="and"<?php if(!$_POST['verify']) echo ' checked' ?>><i class="fas fa-tags fa-fw"></i>AND（全て一致）</label>
<label for="or" class="item"><input type="radio" name="verify" value="1" class="margin line2" id="or"<?php if($_POST['verify']) echo ' checked' ?>><i class="fas fa-tag fa-fw"></i>OR（部分一致）</label><span class="item">［name="verify"］</span>
<?php if($_POST['search']) echo "<br>\n<br>\n" . $result . "\n" ?>
</div>
<div class="box">
...
<div class="radio">
<label for="view" class="item"><input type="radio" name="mode" value="0" class="margin line3" id="view"<?php if($_POST['mode'] == 0) echo ' checked' ?>><i class="fas fa-tablet-alt fa-fw"></i>閲覧</label>&nbsp;
<label for="select" class="item"><input type="radio" name="mode" value="1" class="margin line3" id="select"<?php if($_POST['mode'] == 1) echo ' checked' ?>><i class="far fa-hand-point-up fa-fw"></i>選択</label>&nbsp;
<label for="edit" class="item"><input type="radio" name="mode" value="2" class="margin line3" id="edit"<?php if($_POST['mode'] == 2) echo ' checked' ?>><i class="fas fa-pencil-alt fa-fw"></i>編集</label>&nbsp;
<label for="multi" class="item"><input type="radio" name="mode" value="3" class="margin line3" id="multi"<?php if($_POST['mode'] == 3) echo ' checked' ?>><i class="fas fa-wrench fa-fw"></i>選択&thinsp;&&thinsp;編集</label>&nbsp;&nbsp;
<label for="temp" class="item"><input type="checkbox" name="temp" value="1" class="margin line3" id="temp"<?php if($_POST['temp']) echo ' checked' ?>><i class="fas fa-microchip fa-fw"></i>$csv[]関連配列</label>
</div>
</div>
<br>
...
<?php
	...
	if($selectAll) { preformatted('$selectAll[] <font>選択されたカテゴリー配列</font>', '#f6f6ff', '', $selectAll); }
	if($selectALLtree) { preformatted('$selectALLtree[] <font>選択されたカテゴリー連想配列</font>', '#f6f6ff', '', $selectALLtree); }
	if($selectFolder) { preformatted('$selectFolder[] <font>選択されたフォルダー配列</font>', '#f6f6ff', '', $selectFolder); }
	if($selectItem) { preformatted('$selectItem[] <font>選択された項目配列</font>', '#f6f6ff', '', $selectItem); }
	...
	if($_POST['save']) { preformatted('$_POST[\'save\'] <font>POSTされたCSV保存用配列</font>', '#f6fff6', '', $_POST['save']); }
	if(isset($_POST['string'])) { preformatted('$check[] <font>$_POST[\'string\']から分割し配列に格納</font>', '', '', $check); }
	if($_POST['search']) { preformatted('$_POST[\'search\'] <font>POSTされたカテゴリー照合配列</font>', '', '', $_POST['search']); }
?>
<?php } elseif($parts == 'Source') { //ソースコード表示 ?>
<script>var elements=document.getElementsByName('source');function radioCheck(num){if(elements[num].checked){elements[num].checked=false;}else{elements[num].checked=true;}}</script>
<script src="https://cdn.rawgit.com/google/code-prettify/master/loader/run_prettify.js?lang=css" defer></script>
<script src="<?php echo $path ?>items/lang-php.js" defer></script>
<link rel="stylesheet" href="<?php echo $path ?>items/choka.css">
...
<?php
	$source = array(
		...
		array('開発用PHP', @file_get_contents($path . 'Develop.php'), '', 'fas fa-file-code', 'lang-php', 'button5'),
		array('開発用CSS', @file_get_contents($path . 'Develop.css'), '', 'fab fa-css3-alt', 'lang-css', 'button5')
	);
...
<span class="agent"><?php echo $_SERVER['HTTP_USER_AGENT'] ?></span><br>
...
?>
<br>
<img src="https://api.qrserver.com/v1/create-qr-code/?data=<?php echo $url ?>&size=66x66&format=svg&ecc=L&color=000000&bgcolor=ffffff&margin=0&qzone=0" class="qrcode">
<!--短縮URL : --><span class="short"><?php echo str_replace('https://','',get_short_url($url)) ?></span>
<?php } ?>
