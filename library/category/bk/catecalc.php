...
if(isset($_POST['category'])) {
	list($string, $check) = array_to_string($_POST['category']);
} elseif($_POST['flag']) {
...
for($current = count($csvTree); $current >= 1; $current--) {
	foreach($csvTree[$current] as $value) {
		foreach($value as $line) {
			...
			$order = ($csvTree[$current + 1][$line]) ? 1 : 0;
		...
		for($current = count($CSVTREE); $current >= 1; $current--) {
			foreach($CSVTREE[$current] as $value) {
				foreach($value as $line) {
					...
					//（-）で value値を分割し一時保存の配列に格納
					$temp = explode('-', $line);
					//配列の底を2とする対数のキーが空でなければ $selectAll に追加
					if($check[$temp[0]][log(hexdec($temp[1]), 2)]) {
						//選択された項目を生成（フォルダは上に、項目は下に追加）
						if($order) {
							$selectFolder[] = $line;
...
//	} else {
//		$category1[''] = '<ul class="padding padding1">' . "\n" . '</ul>' . "\n";
//	}
