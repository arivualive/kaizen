$(function() {
	...
	$('#toggle').on('change', function() {
		if($('#toggle').prop('checked')) {
			$('#text').html('<i class="fas fa-pencil-alt fa-fw"></i>Edit mode').css('color', '#b7472a');
			...
			if($('#toggle').data('mode') == 2) {
				$('#text').html('<i class="fas fa-tablet-alt fa-fw"></i>Edit mode').css('color', '');
			} else {
				$('#text').html('<i class="far fa-hand-point-up fa-fw"></i>Selection mode').css('color', '');
			}
	...
	$(document).on('focus', '.label', function() {
		...
		if(!$(this).text()) {
			...
			if(num > limit) {
				$(this).text('Name not set');
			} else {
				$(this).text('No,' + num + 'Category');
			}
		}
		//隠しタグの「value」値を取得・分割
	...
	$(document).on('click', '.add1, .add2', function() {
		...
		if($(this).hasClass('add1')) {
			...
			$(this).before(
				...
				'<input type="hidden" name="save[]" value="' + num + '{}' + newvalue + '{}' + $(this).data('parent') + '{}Name not set{}">' +
				'<input type="checkbox" name="category[]" value="' + newvalue + '"' + data + tag1 + '>' +
				'<label class="label' + tag2 + ' label4"' + style + '>Name not set</label>' +
			...
			$(this).before(
				...
				'<input type="hidden" name="save[]" value="' + num + '{}' + newvalue + '{}' + $(this).data('parent') + '{}No,' + num + 'Category{}">' +
				'<input type="checkbox" name="category[]" value="' + newvalue + '"' + data + tag1 + '>' +
				'<label class="label' + tag2 + ' label' + num + '"' + style + '>No,' + num + 'Category</label>' +
	...
	$(document).on('click', '.del1, .del2', function() {
		...
		if($(this).hasClass('del1')) {
			...
			//子カテゴリーあり
			if(confirm('「' + name + '」Do you want to delete it? Items under the hierarchy are also deleted')) {
...
*/
	...
	$('input[type=checkbox]').on('change', function() {
		$(window).on('beforeunload', function() {
			return 'Changes made are not saved.';
		});
	});
	...
	$('#copy').on('click', function() {
		...
		if(retVal) {
			$('#alert').text('\u00a0Copied to clipboard').addClass('true');
			$('input[name="string"]').focus().select();
		} else {
			$('#alert').text('\u00a0Missing group character').addClass('false');
		}
