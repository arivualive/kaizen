        ...
        if ($this->historyData()) {
            foreach ((array) $this->historyData() as $key => $value) {
                ...
                if ($quiz['qualifying_score'] == 0) {
                    $item  = $active;
                    $item .= sprintf('<a href="result.php?id=%s&an=%s&bid=%s">%dTimes<span class="success">―</span></a>',
                    ...
                    if ($quiz['qualifying_score'] > $value['total_score']) {
                        $item  = $active;
                        $item .= sprintf('<a href="result.php?id=%s&an=%s&bid=%s">%dTimes<span class="fail">failure</span></a>',
                                $value['quiz_id'], $value['answer_id'], $bid, $key+1);
                        $item .= "</li>";
                    }
                    ...
                    if ($quiz['qualifying_score'] <= $value['total_score']) {
                        $item  = $active;
                        $item .= sprintf('<a href="result.php?id=%s&an=%s&bid=%s">%dTimes<span class="success">Pass</span></a>',
                                $value['quiz_id'], $value['answer_id'], $bid, $key+1);
                        $item .= "</li>";
                    }
                }
    ...
    public function showCorrect($answer_query)
    {
        ...
        foreach ((array) $answer_query as $key => $value) {
            ...
            if ($value['flg_right'] == 1) {
                $tag[$key] = '<p class="result correct">Correct answer</p>';
            } else {
                $tag[$key] = '<p class="result incorrect">Incorrect answer</p>';
            }
        }
            ...
            foreach ($selection_data as $i => $value) {
                foreach ($value as $j => $item) {
                    ...
                    if (in_array($item['selection_id'], $choice)) {
                        $tag['choice_class'] = '<td class="user-answer"><span style="font-size:80%">Your answer</span></td>';
                    } else {
