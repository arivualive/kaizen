...
{
$uid=intval($_GET['uid']);
$query=mysqli_query($con,"update tblcms_posts set Is_Active=1 where id='$uid'");
if($query)
{
...
$query=mysqli_query($con,"delete from  tblcms_posts  where id='$id'");
                  ...
                  <div class="card-box">
                    <div class="table-responsive">
                      <table id="example2" class="display table table-striped table-bordered"
                        ...
                        <tbody>
                          <?php
$query=mysqli_query($con,"select tblcms_posts.id as uiid,tblcms_posts.PostTitle as title,tblcms_posts.CreationDate as creationdate,tblcms_posts.Updationdate as updationdate from tblcms_posts where tblcms_posts.Is_Active=0");
...
?>
                          <tr>
                            ...
                            <td>
                              <a href="news-trash-post.php?uid=<?php echo htmlentities($row['uiid']);?>&&action=restore" onclick="return confirm('<?php echo $lang['trash_post_restore_confirm']?>;')"> 
                                <i class=ion-arrow-return-right title="<?php echo  $lang['trash_post_restore_title']?>">
                                </i>
                              </a>
                              &nbsp;
                              <a href="news-trash-post.php?presid=<?php echo htmlentities($row['uiid']);?>&&action=perdel" onclick="return confirm('<?php echo $lang['trash_post_delete']?>;')">
