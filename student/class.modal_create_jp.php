...
class modalCreate {
    ...
    switch ( $data[ 'type' ] ) {
      case 0:
        ...
        if ( $data[ 'contents_extension_id' ] < 7 ) {
          $data[ 'class' ] = "type tb";
          $data[ 'type_string' ] = '動画授業(TB形式)';
        } else {
          $data[ 'class' ] = 'type mp4';
          $data[ 'type_string' ] = '動画授業(MP4形式)';
        }
  ...
  public function modal_display ( $data ) {
    ...
    if ( isset( $data[ 'skip_contents' ] )) {
      ...
      //$s_count = count( $data[ 'skip_contents' ] );
      $c_msg = "関連付けのある下記のコンテンツは受講済です。";
      $q_msg = "その次のコンテンツを受講しますか？";
      $s_title = "<span class='finish'>";

      for ( $s = 0; $s < $s_count; $s++ ) {

        if ( $data[ 'skip_contents' ][ $s ][ 'type' ] == 0 ) {
          $s_title .= "<span class='type movie'>動画授業</span> ". $data[ 'skip_contents' ][ $s ][ 'title' ]."<br>";
        } else if ( $data[ 'skip_contents' ][ $s ][ 'type' ] == 1 ) {
          $s_title .= "<span class='type questionnaire'>アンケート</span> ". $data[ 'skip_contents' ][ $s ][ 'title' ]."<br>";
        } else if ( $data[ 'skip_contents' ][ $s ][ 'type' ] == 2 ) {
          $s_title .= "<span class='type report'>レポート</span>". $data[ 'skip_contents' ][ $s ][ 'title' ]."<br>";
        } else {
          $s_title .= "<span class='type quiz'>テスト</span>". $data[ 'skip_contents' ][ $s ][ 'title' ]."<br>";
        }
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <!-- <p class="type mp4">動画授業(MP4形式)</p> -->
                    <!-- <p class="type test">テスト</p> -->
                    <!-- <p class="type questionnaire">アンケート</p> -->
                    <!-- <p class="type report">レポート</p> -->
                    <p class="title">'.$data['title'].'</p>
                    <ul class="btns">
                      <li><button class="ok" data-next='.$data['url'].'>受講する</button></li>
                      '.$quiz_result.'
